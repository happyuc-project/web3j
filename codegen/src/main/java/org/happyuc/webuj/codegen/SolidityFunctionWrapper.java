...
public class SolidityFunctionWrapper extends Generator {
    ...
    private static final String CODEGEN_WARNING = "<p>Auto generated code.\n"
            ...
            + "<p>Please use the "
            + "<a href=\"https://docs.webuj.io/command_line.html\">webuj command line tools</a>,\n"
            + "or the " + SolidityFunctionWrapperGenerator.class.getName() + " in the \n"
            + "<a href=\"https://github.com/happyuc-project/webu.java/tree/master/codegen\">"
    ...
    private static MethodSpec buildConstructor(Class authType, String authName) {
        return MethodSpec.constructorBuilder()
                ...
                .addParameter(webuj.class, webuj)
                .addParameter(authType, authName)
                .addParameter(BigInteger.class, GAS_PRICE)
                .addParameter(BigInteger.class, GAS_LIMIT)
                .addStatement("super($N, $N, $N, $N, $N, $N)",
                        BINARY, CONTRACT_ADDRESS, webuj, authName, GAS_PRICE, GAS_LIMIT)
                .build();
    }
    ...
    private static MethodSpec buildDeployWithParams(
        ...
        if (isPayable) {
            methodBuilder.addStatement(
                    ...
                    className, webuj, authName, GAS_PRICE, GAS_LIMIT, BINARY, INITIAL_VALUE);
        } else {
            methodBuilder.addStatement(
                    "return deployRemoteCall($L.class, $L, $L, $L, $L, $L, encodedConstructor)",
                    className, webuj, authName, GAS_PRICE, GAS_LIMIT, BINARY);
        }
    ...
    private static MethodSpec buildLoad(
        ...
        return MethodSpec.methodBuilder("load")
                ...
                .addStatement("return new $L($L, $L, $L, $L, $L)", className,
                        CONTRACT_ADDRESS, webuj, authName, GAS_PRICE, GAS_LIMIT)
                .build();
    }
        ...
        if (parameterSpec.type instanceof ParameterizedTypeName) {
            ...
            if (typeNames.size() != 1) {
                ...
                return "new " + parameterSpecType + "(\n"
                        + "        org.happyuc.webuj.abi.Utils.typeMap("
    ...
    private void buildConstantFunction(
        ...
        if (outputParameterTypes.isEmpty()) {
            ...
            ParameterizedTypeName parameterizedTupleType = ParameterizedTypeName.get(
                    ClassName.get(
                            "org.happyuc.webuj.tuples.generated",
    ...
    TypeSpec buildEventResponseObject(
            String className,
            List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> indexedParameters,
            List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> nonIndexedParameters) {

        TypeSpec.Builder builder = TypeSpec.classBuilder(className)
                .addModifiers(Modifier.PUBLIC, Modifier.STATIC);

        builder.addField(LOG, "log", Modifier.PUBLIC);
        for (org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName
    ...
    MethodSpec buildEventObservableFunction(
        ...
        MethodSpec.Builder observableMethodBuilder =
                MethodSpec.methodBuilder(generatedFunctionName)
                        .addModifiers(Modifier.PUBLIC)
                        .addParameter(HucFilter.class, FILTER)
        ...
        observableMethodBuilder
                .addStatement("return webuj.hucLogObservable(filter).map($L)", converter);
    ...
    MethodSpec buildDefaultEventObservableFunction(
        ...
        observableMethodBuilder.addStatement("$1T filter = new $1T($2L, $3L, "
                + "getContractAddress())", HucFilter.class, START_BLOCK, END_BLOCK)
        ...
        for (AbiDefinition.NamedType namedType : inputs) {
            ...
            if (namedType.isIndexed()) {
                indexedParameters.add(
                        new org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName(
                                namedType.getName(),
