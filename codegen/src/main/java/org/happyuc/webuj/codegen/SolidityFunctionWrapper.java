...
import org.happyuc.webuj.abi.datatypes.generated.AbiTypes;
...
import org.happyuc.webuj.utils.Version;
...
public class SolidityFunctionWrapper extends Generator {
    ...
    private static final String WEBUJ = "webuj";
    ...
    private void addAddressesSupport(TypeSpec.Builder classBuilder, Map<String, String> addresses) {
        if (addresses != null) {
            ...
            FieldSpec addressesStaticField = FieldSpec.builder(mapStringString, "_addresses", Modifier.PROTECTED, Modifier.STATIC, Modifier.FINAL).build();
            ...
            for (String key : addresses.keySet()) {
                staticInit.addStatement(String.format("_addresses.put(\"%1s\", \"%2s\")", key, addresses.get(key)));
            }
            classBuilder.addStaticBlock(staticInit.build());

            // See org.happyuc.webuj.tx.Contract#getStaticDeployedAddress(String)
            MethodSpec getAddress = MethodSpec.methodBuilder("getStaticDeployedAddress").addModifiers(Modifier.PROTECTED).returns(stringType).addParameter(stringType, "networkId").addCode(CodeBlock.builder().addStatement("return _addresses.get(networkId)").build()).build();
            classBuilder.addMethod(getAddress);

            MethodSpec getPreviousAddress = MethodSpec.methodBuilder("getPreviouslyDeployedAddress").addModifiers(Modifier.PUBLIC).addModifiers(Modifier.STATIC).returns(stringType).addParameter(stringType, "networkId").addCode(CodeBlock.builder().addStatement("return _addresses.get(networkId)").build()).build();
    ...
    private TypeSpec.Builder createClassBuilder(String className, String binary) {
        ...
        String javadoc = CODEGEN_WARNING + getWebujVersion();

        return TypeSpec.classBuilder(className).addModifiers(Modifier.PUBLIC).addJavadoc(javadoc).superclass(Contract.class).addField(createBinaryDefinition(binary));
    }

    private String getWebujVersion() {
        String version;

        try {
            // This only works if run as part of the webuj command line tools which contains
        ...
        return "\n<p>Generated with webuj version " + version + ".\n";
    }

    private FieldSpec createBinaryDefinition(String binary) {
        return FieldSpec.builder(String.class, BINARY).addModifiers(Modifier.PRIVATE, Modifier.FINAL, Modifier.STATIC).initializer("$S", binary).build();
    }
    ...
    private static MethodSpec buildConstructor(Class authType, String authName) {
        return MethodSpec.constructorBuilder().addModifiers(Modifier.PROTECTED).addParameter(String.class, CONTRACT_ADDRESS).addParameter(Webuj.class, WEBUJ).addParameter(authType, authName).addParameter(BigInteger.class, GAS_PRICE).addParameter(BigInteger.class, GAS_LIMIT).addStatement("super($N, $N, $N, $N, $N, $N)", BINARY, CONTRACT_ADDRESS, WEBUJ, authName, GAS_PRICE, GAS_LIMIT).build();
    }
    ...
    private static MethodSpec buildDeployWithParams(MethodSpec.Builder methodBuilder, String className, String inputParams, String authName, boolean isPayable) {
        ...
        if (isPayable) {
            methodBuilder.addStatement("return deployRemoteCall($L.class, $L, $L, $L, $L, $L, encodedConstructor, $L)", className, WEBUJ, authName, GAS_PRICE, GAS_LIMIT, BINARY, INITIAL_VALUE);
        } else {
            methodBuilder.addStatement("return deployRemoteCall($L.class, $L, $L, $L, $L, $L, encodedConstructor)", className, WEBUJ, authName, GAS_PRICE, GAS_LIMIT, BINARY);
        }
    ...
    private static MethodSpec buildDeployNoParams(MethodSpec.Builder methodBuilder, String className, String authName, boolean isPayable) {
        if (isPayable) {
            methodBuilder.addStatement("return deployRemoteCall($L.class, $L, $L, $L, $L, $L, \"\", $L)", className, WEBUJ, authName, GAS_PRICE, GAS_LIMIT, BINARY, INITIAL_VALUE);
        } else {
            methodBuilder.addStatement("return deployRemoteCall($L.class, $L, $L, $L, $L, $L, \"\")", className, WEBUJ, authName, GAS_PRICE, GAS_LIMIT, BINARY);
        }
    ...
    private static MethodSpec.Builder getDeployMethodSpec(String className, Class authType, String authName, boolean isPayable) {
        MethodSpec.Builder builder = MethodSpec.methodBuilder("deploy").addModifiers(Modifier.PUBLIC, Modifier.STATIC).returns(buildRemoteCall(TypeVariableName.get(className, Type.class))).addParameter(Webuj.class, WEBUJ).addParameter(authType, authName).addParameter(BigInteger.class, GAS_PRICE).addParameter(BigInteger.class, GAS_LIMIT);
        if (isPayable) {
    ...
    private static MethodSpec buildLoad(String className, Class authType, String authName) {
        return MethodSpec.methodBuilder("load").addModifiers(Modifier.PUBLIC, Modifier.STATIC).returns(TypeVariableName.get(className, Type.class)).addParameter(String.class, CONTRACT_ADDRESS).addParameter(Webuj.class, WEBUJ).addParameter(authType, authName).addParameter(BigInteger.class, GAS_PRICE).addParameter(BigInteger.class, GAS_LIMIT).addStatement("return new $L($L, $L, $L, $L, $L)", className, CONTRACT_ADDRESS, WEBUJ, authName, GAS_PRICE, GAS_LIMIT).build();
    }
    ...
    String addParameters(MethodSpec.Builder methodBuilder, List<AbiDefinition.NamedType> namedTypes) {
        ...
        List<ParameterSpec> nativeInputParameterTypes = new ArrayList<ParameterSpec>(inputParameterTypes.size());
            ...
            return Collection.join(inputParameterTypes, ", \n",
                    ...
                    new Collection.Function<ParameterSpec, String>() {
                        ...
                        public String apply(ParameterSpec parameterSpec) {
                            return SolidityFunctionWrapper.this.createMappedParameterTypes(parameterSpec);
                        }
                    });
        } else {
            return Collection.join(inputParameterTypes, ", ", new Collection.Function<ParameterSpec, String>() {
                @Override
                public String apply(ParameterSpec parameterSpec) {
    ...
    private String createMappedParameterTypes(ParameterSpec parameterSpec) {
        if (parameterSpec.type instanceof ParameterizedTypeName) {
            ...
            if (typeNames.size() != 1) {
                ...
                return "new " + parameterSpec.type + "(\n" + "        org.happyuc.webuj.abi.Utils.typeMap(" + parameterSpec.name + ", " + typeName + ".class)" + ")";
            }
        } else {
    ...
    private void buildConstantFunction(AbiDefinition functionDefinition, MethodSpec.Builder methodBuilder, List<TypeName> outputParameterTypes, String inputParams) throws ClassNotFoundException {
        ...
        if (outputParameterTypes.isEmpty()) {
            ...
            methodBuilder.addStatement("final $T function = " + "new $T($S, \n$T.<$T>asList($L), " + "\n$T.<$T<?>>asList(new $T<$T>() {}))", Function.class, Function.class, functionName, Arrays.class, Type.class, inputParams, Arrays.class, TypeReference.class, TypeReference.class, typeName);
            ...
            if (useNativeJavaTypes) {
                if (nativeReturnTypeName.equals(ClassName.get(List.class))) {
                    ...
                    callCode.addStatement("$T result = " + "($T) executeCallSingleValueReturn(function, $T.class)", listType, listType, nativeReturnTypeName);
                    callCode.addStatement("return convertToNative(result)");

                    TypeSpec callableType = TypeSpec.anonymousClassBuilder("").addSuperinterface(ParameterizedTypeName.get(ClassName.get(Callable.class), nativeReturnTypeName)).addMethod(MethodSpec.methodBuilder("call").addAnnotation(Override.class).addAnnotation(AnnotationSpec.builder(SuppressWarnings.class).addMember("value", "$S", "unchecked").build()).addModifiers(Modifier.PUBLIC).addException(Exception.class).returns(nativeReturnTypeName).addCode(callCode.build()).build()).build();

                    methodBuilder.addStatement("return new $T(\n$L)", buildRemoteCall(nativeReturnTypeName), callableType);
                } else {
                    methodBuilder.addStatement("return executeRemoteCallSingleValueReturn(function, $T.class)", nativeReturnTypeName);
                }
            } else {
            ...
            ParameterizedTypeName parameterizedTupleType = ParameterizedTypeName.get(ClassName.get("org.happyuc.webuj.tuples.generated", "Tuple" + returnTypes.size()), returnTypes.toArray(new TypeName[returnTypes.size()]));
    ...
    TypeSpec buildEventResponseObject(String className, List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> indexedParameters, List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> nonIndexedParameters) {
        ...
        for (org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName namedType : indexedParameters) {
        ...
        for (org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName namedType : nonIndexedParameters) {
    ...
    MethodSpec buildEventObservableFunction(String responseClassName, String functionName, List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> indexedParameters, List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> nonIndexedParameters) throws ClassNotFoundException {
        ...
        TypeSpec converter = TypeSpec.anonymousClassBuilder("").addSuperinterface(ParameterizedTypeName.get(ClassName.get(Func1.class), ClassName.get(Log.class), ClassName.get("", responseClassName))).addMethod(MethodSpec.methodBuilder("call").addAnnotation(Override.class).addModifiers(Modifier.PUBLIC).addParameter(Log.class, "log").returns(ClassName.get("", responseClassName)).addStatement("$T eventValues = extractEventParametersWithLog(event, log)", Contract.EventValuesWithLog.class).addStatement("$1T typedResponse = new $1T()", ClassName.get("", responseClassName)).addCode(buildTypedResponse("typedResponse", indexedParameters, nonIndexedParameters, true)).addStatement("return typedResponse").build()).build();

        observableMethodBuilder.addStatement("$1T filter = new $1T($2L, $3L, " + "getContractAddress())", HucReqFilter.class, START_BLOCK, END_BLOCK).addStatement("filter.addSingleTopic($T.encode(event))", EventEncoder.class).addStatement("return webuj.hucLogObservable(filter).map($L)", converter);

        return observableMethodBuilder.build();
    }

    MethodSpec buildEventTransactionReceiptFunction(String responseClassName, String functionName, List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> indexedParameters, List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> nonIndexedParameters) throws ClassNotFoundException {
    ...
    void buildEventFunctions(AbiDefinition functionDefinition, TypeSpec.Builder classBuilder) throws ClassNotFoundException {
        ...
        List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> indexedParameters = new ArrayList<NamedTypeName>();
        List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> nonIndexedParameters = new ArrayList<NamedTypeName>();
        for (AbiDefinition.NamedType namedType : inputs) {

            if (namedType.isIndexed()) {
                indexedParameters.add(new org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName(namedType.getName(), buildTypeName(namedType.getType())));
            } else {
                nonIndexedParameters.add(new org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName(namedType.getName(), buildTypeName(namedType.getType())));
            }
        }

        classBuilder.addType(buildEventResponseObject(responseClassName, indexedParameters, nonIndexedParameters));

        classBuilder.addMethod(buildEventTransactionReceiptFunction(responseClassName, functionName, indexedParameters, nonIndexedParameters));
        classBuilder.addMethod(buildEventObservableFunction(responseClassName, functionName, indexedParameters, nonIndexedParameters));
    }

    CodeBlock buildTypedResponse(String objectName, List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> indexedParameters, List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> nonIndexedParameters, boolean observable) {
    ...
    private static Class<?> getStaticArrayTypeReferenceClass(String[] splitType) {
        try {
            return Class.forName("org.happyuc.webuj.abi.datatypes.generated.StaticArray" + splitType[1]);
    ...
    private static void buildVariableLengthReturnFunctionConstructor(MethodSpec.Builder methodBuilder, String functionName, String inputParameters, List<TypeName> outputParameterTypes) throws ClassNotFoundException {
        ...
        String asListParams = Collection.join(outputParameterTypes, ", ", new Collection.Function<TypeName, String>() {
            @Override
    ...
    private static void buildVariableLengthEventConstructor(MethodSpec.Builder methodBuilder, String eventName, List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> indexedParameterTypes, List<org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName> nonIndexedParameterTypes) throws ClassNotFoundException {
        ...
        objects.add(Arrays.class);
        objects.add(TypeReference.class);
        for (org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName indexedParameterType : indexedParameterTypes) {
            objects.add(TypeReference.class);
        ...
        objects.add(Arrays.class);
        objects.add(TypeReference.class);
        for (org.happyuc.webuj.codegen.SolidityFunctionWrapper.NamedTypeName indexedParameterType : nonIndexedParameterTypes) {
            objects.add(TypeReference.class);
            objects.add(indexedParameterType.getTypeName());
        }

        String indexedAsListParams = Collection.join(indexedParameterTypes, ", ", new Collection.Function<NamedTypeName, String>() {
            @Override
        ...
        String nonIndexedAsListParams = Collection.join(nonIndexedParameterTypes, ", ", new Collection.Function<NamedTypeName, String>() {
            @Override
            public String apply(NamedTypeName typeName) {
                return "new $T<$T>() {}";
            }
